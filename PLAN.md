...
## Текущий статус (обновлено 2026-09-23)

Подробности — в [CHANGELOG.md](CHANGELOG.md).

- ✅ **Этап 0** — готов.
- 🟡 **Этап 1** — сделаны UI-kit (доски 04–07) и страница проекта: шапка с мини-шапкой, первый блок,
  галерея с липким aside, слайдер «Реализованные проекты», подвал. Блок плана — заглушка (этап 3).
  **Осталось:** форма «Отправить проект» (доска 06), главная (фреймы «Главная»), страница `/projects`,
  юридические страницы, лайтбокс галереи; раскрытое мобильное меню и планшет — ждут макета.
- ⬜ Этапы 2–6 — не начаты.
...
- 🟡 Страница «Проект» (`23:2` / `27:254`) — всё, кроме формы; «Главная» — не начата. Брейкпоинты 1440 → 360 (планшет — интерполяция).
- 🟡 Слайдер проектов и липкая колонка галереи — готовы; лайтбокс — нет.
...
- ⚠️ Витрина на GitHub Pages собирается статически без сервера и БД. Когда страницы перейдут на
  Payload Local API, нужно решить заранее: либо сборка Pages поднимает Postgres + seed в CI и берёт
  данные на этапе build (`generateStaticParams`), либо витрина переезжает на staging VPS.
